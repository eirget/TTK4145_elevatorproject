...
procedure protectobj is
    ...
    -- Endringer starter her: 
    protected type Resource is -- Her defineres all funksjonene som skal lages inne i samme resource
        entry allocateHigh(val: out IntVec.Vector); -- Disse to er entry fordi de er beskyttet av entrypoints
        entry allocateLow(val: out IntVec.Vector);  -- ^
        procedure deallocate(val: IntVec.Vector);   -- procedure fordi det skal skje etter entry
    private -- Her defineres variablene som skal tas med videre
        value: IntVec.Vector;
        busy: Boolean := False;
    end Resource;
    protected body Resource is
    
        -- Her er det satt på en betingelse etter funksjonsnavnet
        -- Hensikten med det er å si at det må være plass til å execute oss og allocateHigh'Count gir antallet som er i kø
        entry allocateLow(val: out IntVec.Vector) when not busy and allocateHigh'Count = 0 is
        -- Når entry er godkjent går vi inn i begin, her sies det egt at det er vår ordre som skal kjøre
        begin
            val := value;
            busy := True;
        end allocateLow;
    
        -- Her er betingelsen at det kun ikke skal være busy. Som betyr at de IDene som kommer inn i allocateHigh legges til i kø før allocateLow, siden de kun har en betingelse
        ...
        -- Denner jeg litt usikker på, men jeg tror
        -- deallocate setter tilstand tilbake til ledig, hver gang en ID har brukt ressursen
