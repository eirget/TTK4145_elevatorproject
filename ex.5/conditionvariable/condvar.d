...
class Resource(T) {
    ...
    T allocate(int id, int priority){
        mtx.lock(); // Create a lock, trenger en lock for å sørge for at ingen andre går inn samtidig som oss
        queue.insert(id, priority); //Setter inn id i queue på den plassen som priority tilsier

        //while sørger for å bli stående her og vente frem til det er vår tur
        //lock sørger allerede for at vi ikke kjemper mot andre om å sette igang noe
        while (queue.front() != id || inUse) {
            cond.wait();
        }

        queue.popFront(); //når vi ikke lenger står i kø, altså det er vår tur, kan vi poppes fra køen. 
        inUse = true; //Sier ifra at vi tar ibruk ressursen, slik at ikke andre får tilgang
        mtx.unlock(); // avslutter med å åpne locken
        return value;
    }
    
    void deallocate(T v){
        mtx.lock(); //sette en lock for å sørge for at det kun skjer en og en ting
        value = v;
        inUse = false; //sier ifra at vi er ferdig med ressursen
        cond.notifyAll(); // releases all of the tasks
        mtx.unlock();
    }
}
